...
// lvmax: 使ってよい拡大率レベルの上限（解像度フィルタ）。255 なら無制限
// anc: 1 ならスロット k がアンカー範囲内の標本だけを使う
fn ok(w0: u32, k: u32, mode: u32, med: vec3<f32>, lvmax: u32, anc: u32) -> bool {
  let fl = w0 >> 24u;
  let geom = (fl & 1u) != 0u; let clean = (fl & 2u) != 0u;
  if (lv_of(w0) > lvmax) { return false; }
  if (anc == 1u && (k < p.anc_lo || k > p.anc_hi)) { return false; }
...
fn kth_value(base: u32, stride: u32, ch: u32, mode: u32, med: vec3<f32>, kth_in: u32, lvmax: u32, anc: u32) -> u32 {
  var kth = kth_in;
  var prefix = 0u;
  ...
  for (var ps = 0u; ps < npass; ps++) {
    ...
    for (var k = 0u; k < p.K; k++) {
      ...
      if (!ok(w0, k, mode, med, lvmax, anc)) { continue; }
      let v = val(w0, w1, ch);
...
fn count_mode(base: u32, stride: u32, mode: u32, med: vec3<f32>, lvmax: u32, anc: u32) -> u32 {
  var n = 0u;
  for (var k = 0u; k < p.K; k++) { if (ok(stack[base + k * stride], k, mode, med, lvmax, anc)) { n++; } }
  return n;
}
...
fn lv_limit(base: u32, stride: u32, mode: u32, anc: u32) -> u32 {
  ...
  var mn = 255u;
  for (var k = 0u; k < p.K; k++) { let w0 = stack[base + k * stride]; if (ok(w0, k, mode, vec3<f32>(0.0), 255u, anc)) { mn = min(mn, lv_of(w0)); } }
...
fn main(@builtin(global_invocation_id) g: vec3<u32>) {
  ...
  let anc_g = select(0u, 1u, p.anc_lo <= p.anc_hi && count_mode(base, stride, 1u, zero, 255u, 1u) > 0u);
  ...
  let mcnt = count_mode(base, stride, mmode, zero, lvm, ancm);
  let kmed = (mcnt - 1u) / 2u;
  let med = vec3<f32>(f32(kth_value(base, stride, 0u, mmode, zero, kmed, lvm, ancm)),
                      f32(kth_value(base, stride, 1u, mmode, zero, kmed, lvm, ancm)),
                      f32(kth_value(base, stride, 2u, mmode, zero, kmed, lvm, ancm)));
  // インライア
  var imode = 2u;
  var ilv = lvc;
  var ianc = anc_c;
  var ni = count_mode(base, stride, 2u, med, lvc, anc_c);
  ...
  if (ni > 0u) {
    ...
    for (var k = 0u; k < p.K; k++) {
      let w0 = stack[base + k * stride]; let w1 = stack[base + k * stride + 1u];
      if (ok(w0, k, imode, med, ilv, ianc) && w1 >= thr) { ts += rgb3(w0); nt++; }
    }
